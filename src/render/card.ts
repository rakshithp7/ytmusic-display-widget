...
export function renderCard(data: CardData, options: RenderOptions): string {
  ...
  return `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="${textX}" y="${height / 2 + 8}" font-family="Poppins, sans-serif" font-size="${titleSize}"
      font-weight="700" fill="#fff">${escapeXml(data.title)}</text>
    <text x="${textX}" y="${height / 2 + 8 + artistSize + 6}" font-family="Poppins, sans-serif" font-size="${artistSize}"
      font-weight="500" fill="rgba(255,255,255,0.75)">${escapeXml(data.artist)}</text>
